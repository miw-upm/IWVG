...
import ticTacToe.v430.controllers.MoveController;
...
public class LocalMoveController extends LocalColocateController implements
	...
	public ErrorReport validateOrigin(Coordinate origin) {
		assert origin != null;
		if (!this.full(origin)) {
			return ErrorGeneratorType.NOT_PROPERTY.getErrorReport(this
					.getGame());
		}
		return null;
	}
	...
	public ErrorReport validateTarget(Coordinate origin, Coordinate target) {
		...
		if (origin.equals(target)) {
			return ErrorGeneratorType.REPEATED_COORDINATE.getErrorReport(this
					.getGame());
		}
		return null;
	}
